...
update bmem.op_detalle_de_pago set doc_id = concat('PI ', doc_id) where doc_id not like '%P%' and concepto = 'TRANSFERENCIA';
update bmem.op_detalle_de_pago set doc_id = concat('PM ', doc_id) where doc_id not like '%P%' and concepto = 'COMPENSACION';

update bmem.op_detalle_de_aplicado set doc_id = a.doc_id from (select doc_id, periodo, cliente, NULLIF(regexp_replace(doc_id, '\D','','g'), '') as x from bmem.op_detalle_de_pago) as a
where NULLIF(regexp_replace(op_detalle_de_aplicado.doc_id, '\D','','g'), '') = a.x and op_detalle_de_aplicado.periodo = a.periodo and op_detalle_de_aplicado.cliente = a.cliente
and op_detalle_de_aplicado.doc_id not like '%P%' ;

update bmem.op_retenciones set doc_id = a.doc_id from (select doc_id, periodo, cliente, NULLIF(regexp_replace(doc_id, '\D','','g'), '') as x from bmem.op_detalle_de_pago) as a
where NULLIF(regexp_replace(op_retenciones.doc_id, '\D','','g'), '') = a.x and op_retenciones.periodo = a.periodo and op_retenciones.cliente = a.cliente
and op_retenciones.doc_id not like '%P%' ;
...
update bmem.op_retenciones set concepto = 'Retención Imp.Ganancias RI - MEM' where concepto like '%Retención Imp.Ganancias RI - MEM%';
update bmem.op_retenciones set concepto = 'Retención de IVA - RG 3130 - MEM' where concepto like '%Retención de IVA - RG 3130 - MEM%';
update bmem.op_retenciones set concepto = 'Retención SUSS RG - MEM' where concepto like '%Retención SUSS RG - MEM%';