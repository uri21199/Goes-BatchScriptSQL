...
BEGIN
    FOR rec IN SELECT * FROM full_db_sources LOOP
        ...
        DELETE FROM rec.delete1 WHERE date >= now() - interval '3 days';
        DELETE FROM rec.delete2 WHERE date >= now() - interval '3 days';
...
BEGIN
    FOR rec IN SELECT * FROM full_db_sources2 LOOP
        ...
        rec.col1 = rec.col1 / 1000 WHERE date >= now() - interval '3 days' AND date <= CURRENT_TIMESTAMP;
    END LOOP;
END $$;
...
DO $$ 
DECLARE
    startdate text := to_char(current_date - 3, 'YYYYMMDD');
...
BEGIN
    FOR rec IN SELECT * FROM trafos_sources LOOP
        ...
        WHERE date >= (now()::timestamptz - interval '3 days');
    END LOOP;
END $$;
