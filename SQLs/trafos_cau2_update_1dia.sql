DELETE FROM alert_cau.caupotenciatrafos_cau2fv WHERE date >= now() - interval '120 minutes';
...
BEGIN
    FOR rec IN SELECT * FROM trafos_sources LOOP
        ...
        EXECUTE format(
            'COPY stage_trafos_cau_2 FROM PROGRAM ''curl -H "Api-Key:38deaa51-8716-4cc9-aa42-ab14fb72a752.01" "%s?&start_date=%sT%sZ"''',
            rec.url, startdate, starttime
        );

        -- Insertar los datos en la tabla temporal y hacer las actualizaciones
        INSERT INTO stage_trafos_cau_22 
        SELECT f.* 
        FROM stage_trafos_cau_2, jsonb_populate_recordset(null::stage_trafos_cau_22, x) f;

        UPDATE stage_trafos_cau_22 
        SET cau = 'cau2fv', 
            id = rec.id,
