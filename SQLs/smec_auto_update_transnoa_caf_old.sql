...
UPDATE tmp_smec_caf SET date = date_part.date2 FROM (SELECT split_part(hour,' ',1) as date2 FROM tmp_smec_caf WHERE hour LIKE '%/%' LIMIT 1) as date_part;
UPDATE tmp_smec_caf SET hour = hour_part.hour2 FROM (SELECT split_part(hour,' ',2) as hour2, hour FROM tmp_smec_caf WHERE hour LIKE '%/%') as hour_part WHERE tmp_smec_caf.hour = hour_part.hour;
...
SELECT * from smec.caf_ids CROSS JOIN generate_series(DATE_TRUNC('hour', now() - interval '1 day') ,DATE_TRUNC('hour', now()), '1 hour')  ON CONFLICT DO NOTHING;
--------------------------------------------------------------------
