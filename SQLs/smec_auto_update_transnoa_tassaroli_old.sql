...
UPDATE tmp_smec_tassaroli SET date = date_part.date2 FROM (SELECT split_part(hour,' ',1) as date2 FROM tmp_smec_tassaroli WHERE hour LIKE '%/%' LIMIT 1) as date_part;
UPDATE tmp_smec_tassaroli SET hour = hour_part.hour2 FROM (SELECT split_part(hour,' ',2) as hour2, hour FROM tmp_smec_tassaroli WHERE hour LIKE '%/%') as hour_part WHERE tmp_smec_tassaroli.hour = hour_part.hour;
