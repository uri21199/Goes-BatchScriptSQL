...
UPDATE tmp_smec_arb SET date = date_part.date2 FROM (SELECT split_part(hour,' ',1) as date2 FROM tmp_smec_arb WHERE hour LIKE '%/%' LIMIT 1) as date_part;
UPDATE tmp_smec_arb SET hour = hour_part.hour2 FROM (SELECT split_part(hour,' ',2) as hour2, hour FROM tmp_smec_arb WHERE hour LIKE '%/%') as hour_part WHERE tmp_smec_arb.hour = hour_part.hour;
...
SELECT * from smec.arb_ids CROSS JOIN generate_series(DATE_TRUNC('hour', now() - interval '20 day') ,DATE_TRUNC('hour', now()), '1 hour')  ON CONFLICT DO NOTHING;
----------------------------------------------------------------------------
