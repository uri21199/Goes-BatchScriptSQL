...
CREATE TEMP TABLE resumen_cau_sources (url text);

-- Insertar las rutas de los archivos CSV en la tabla temporal
INSERT INTO resumen_cau_sources (url)
VALUES
...
DO $$ 
BEGIN
    FOR rec IN SELECT * FROM resumen_cau_sources LOOP
        ...
        EXECUTE format(
            'COPY tmp_ocr_cau FROM %L WITH (FORMAT CSV, DELIMITER '';'', NULL ''nan'', HEADER)',
            rec.url
        );

        -- Insertar los datos finales en la tabla bmem.cau_resumen
        INSERT INTO bmem.cau_resumen  
        SELECT * FROM tmp_ocr_cau 
        ON CONFLICT ON CONSTRAINT uc_cau_resumen DO NOTHING;
    END LOOP;
END $$;

-- Eliminar la tabla temporal de fuentes
DROP TABLE resumen_cau_sources;