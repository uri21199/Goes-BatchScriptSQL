...
CREATE TEMP TABLE file_paths_caf (
    id serial PRIMARY KEY,
    file_path text
);

-- Insertar las rutas de los archivos CSV en la tabla auxiliar
INSERT INTO file_paths_caf (file_path) VALUES
...
DECLARE 
    file_record record;
BEGIN
    FOR file_record IN (SELECT file_path FROM file_paths_caf) LOOP
        EXECUTE format('
            COPY tmp_ocr_caf FROM %L DELIMITER '';'' NULL AS ''NaN'' CSV HEADER', file_record.file_path);
    END LOOP;
END $$;
...
INSERT INTO bmem.cafayate_detalle 
SELECT 
    cantidad, 
    detalle, 
    importe, 
