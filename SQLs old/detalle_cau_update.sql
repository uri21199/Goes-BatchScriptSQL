...
CREATE TEMP TABLE tmp_ocr_cau (n text, cantidad numeric, n2 text, detalle text, importe numeric, ajuste_doc text, tipo_doc text, num_doc text, emision date, vencimiento date, agente text, periodo text,id integer);
...
ALTER TABLE tmp_ocr_cau DROP COLUMN n, DROP COLUMN n2, DROP COLUMN periodo;
UPDATE tmp_ocr_cau SET detalle = 'Remuneracion Base Energia No Convencionales' WHERE detalle like  'Remu%';
UPDATE tmp_ocr_cau SET detalle = 'Valorizacion Energia - Cargo Variable' WHERE detalle like  'Valo%';
INSERT INTO bmem.cauchari_detalle SELECT cantidad , detalle , importe , ajuste_doc , tipo_doc , num_doc , emision , vencimiento , agente , periodo2 , id  from tmp_ocr_cau ON CONFLICT DO NOTHING;
