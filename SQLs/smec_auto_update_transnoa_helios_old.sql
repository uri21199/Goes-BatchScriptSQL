...
UPDATE tmp_smec_helios SET date = date_part.date2 FROM (SELECT split_part(hour,' ',1) as date2 FROM tmp_smec_helios WHERE hour LIKE '%/%' LIMIT 1) as date_part;
UPDATE tmp_smec_helios SET hour = hour_part.hour2 FROM (SELECT split_part(hour,' ',2) as hour2, hour FROM tmp_smec_helios WHERE hour LIKE '%/%') as hour_part WHERE tmp_smec_helios.hour = hour_part.hour;
