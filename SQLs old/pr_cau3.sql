ALTER TABLE pr_cau3 ADD CONSTRAINT uc_pr_cau3 UNIQUE (date);
...
COPY stage_pr_cau3 FROM PROGRAM 'curl "https://portal.unity.inaccess.com/ifms/sources/125ee3d8-07ed-11eb-8564-42010afa015a/events?apiKey=38deaa51-8716-4cc9-aa42-ab14fb72a752.01&start_date=20221025T000000Z"';
...
CREATE TEMP TABLE stage_pr_cau3(x jsonb);
CREATE TEMP TABLE stage_pr_cau32(date timestamptz, val numeric);
COPY stage_pr_cau3 FROM PROGRAM 'curl "https://portal.unity.inaccess.com/ifms/sources/125ad5ea-07ed-11eb-8564-42010afa015a/events?apiKey=38deaa51-8716-4cc9-aa42-ab14fb72a752.01&start_date=20221025T000000Z"';
INSERT INTO stage_pr_cau32 select f.* from stage_pr_cau3, jsonb_populate_recordset(null::stage_pr_cau32, x) f;
UPDATE stage_pr_cau32 SET date = DATE_TRUNC('minute', date);
INSERT INTO pr_ii3 SELECT * from stage_pr_cau32;
...
CREATE TEMP TABLE stage_pr_cau3(x jsonb);
CREATE TEMP TABLE stage_pr_cau32(date timestamptz, val numeric);
COPY stage_pr_cau3 FROM PROGRAM 'curl "https://portal.unity.inaccess.com/ifms/sources/12261f8a-07ed-11eb-8564-42010afa015a/events?apiKey=38deaa51-8716-4cc9-aa42-ab14fb72a752.01&start_date=20221025T000000Z"';
INSERT INTO stage_pr_cau32 select f.* from stage_pr_cau3, jsonb_populate_recordset(null::stage_pr_cau32, x) f;
UPDATE stage_pr_cau32 SET date = DATE_TRUNC('minute', date);
INSERT INTO pr_ii3 SELECT * from stage_pr_cau32;
...
CREATE TEMP TABLE stage_pr_cau3(x jsonb);
CREATE TEMP TABLE stage_pr_cau32(date timestamptz, val numeric);
COPY stage_pr_cau3 FROM PROGRAM 'curl "https://portal.unity.inaccess.com/ifms/sources/12217976-07ed-11eb-8564-42010afa015a/events?apiKey=38deaa51-8716-4cc9-aa42-ab14fb72a752.01&start_date=20221025T000000Z"';
INSERT INTO stage_pr_cau32 select f.* from stage_pr_cau3, jsonb_populate_recordset(null::stage_pr_cau32, x) f;
UPDATE stage_pr_cau32 SET date = DATE_TRUNC('minute', date);
INSERT INTO pr_ii3 SELECT * from stage_pr_cau32;
...
CREATE TEMP TABLE stage_pr_cau3(x jsonb);
CREATE TEMP TABLE stage_pr_cau32(date timestamptz, val numeric);
COPY stage_pr_cau3 FROM PROGRAM 'curl "https://portal.unity.inaccess.com/ifms/sources/11ecfa70-07ed-11eb-8564-42010afa015a/events?apiKey=38deaa51-8716-4cc9-aa42-ab14fb72a752.01&start_date=20221025T000000Z"';
INSERT INTO stage_pr_cau32 select f.* from stage_pr_cau3, jsonb_populate_recordset(null::stage_pr_cau32, x) f;
UPDATE stage_pr_cau32 SET date = DATE_TRUNC('minute', date);
INSERT INTO pr_ii3 SELECT * from stage_pr_cau32;
...
CREATE TEMP TABLE stage_pr_cau3(x jsonb);
CREATE TEMP TABLE stage_pr_cau32(date timestamptz, val numeric);
COPY stage_pr_cau3 FROM PROGRAM 'curl "https://portal.unity.inaccess.com/ifms/sources/11e80718-07ed-11eb-8564-42010afa015a/events?apiKey=38deaa51-8716-4cc9-aa42-ab14fb72a752.01&start_date=20221025T000000Z"';
INSERT INTO stage_pr_cau32 select f.* from stage_pr_cau3, jsonb_populate_recordset(null::stage_pr_cau32, x) f;
UPDATE stage_pr_cau32 SET date = DATE_TRUNC('minute', date);
INSERT INTO pr_ii3 SELECT * from stage_pr_cau32;
...
FROM   pr_cau3 WHERE date >= (now() - interval '450 days')) AS g_average_calc WHERE g_average_calc.date=pr_cau3.date;
	...
	FROM (SELECT date,
       (SELECT MAX(b)::numeric (10, 4)
        FROM   (VALUES(WS_C29),
                      (WS_C11),
                      (WS_C08)) T (b)) AS g_max
FROM   pr_cau3 WHERE date >= (now() - interval '450 days')) AS g_max_calc WHERE g_max_calc.date=pr_cau3.date;
	...
	FROM (SELECT date,
       (SELECT Min(c)::numeric (10, 4)
        FROM   (VALUES(WS_C29),
                      (WS_C11),
                      (WS_C08)) T (c)) AS g_min
FROM   pr_cau3 WHERE date >= (now() - interval '450 days')) AS g_min_calc WHERE g_min_calc.date=pr_cau3.date;


UPDATE pr_cau3 SET max_diff = max_diff_calc.max_diff FROM (SELECT date, ABS((g_max / COALESCE(NULLIF(g_min,0), 1))-1) AS max_diff FROM pr_cau3 WHERE date >= (now() - interval '450 days')) AS max_diff_calc WHERE max_diff_calc.date=pr_cau3.date;

UPDATE pr_cau3 SET g_seleccionado = g_average WHERE date >= (now() - interval '450 days') AND max_diff <= 0.02;
UPDATE pr_cau3 SET g_seleccionado = g_max WHERE date >= (now() - interval '450 days') AND max_diff > 0.02;

UPDATE pr_cau3 SET g_average_m2 = g_average/60 WHERE date >= (now() - interval '450 days'); 

-------------------15min---------------------
DROP TABLE IF EXISTS pr_15min_cau3; 
CREATE TABLE pr_15min_cau3 (date_15min timestamptz, g_15min_average numeric (10, 4));

INSERT INTO pr_15min_cau3(date_15min) SELECT (to_timestamp(ceiling(( extract ('epoch' from (date)) ) / (60*15) ) * (60*15))) AS date_15min from pr_cau3 WHERE date >= (now() - interval '450 days') GROUP BY date_15min;
...
UPDATE pr_15min_cau3 SET g_15min_average = average_15min_calc.average_15min 
	FROM (SELECT 
		...
		(to_timestamp(ceiling(( extract ('epoch' from (date)) ) / (60*15) ) * (60*15))) AS date_15min
			from pr_cau3 WHERE date >= (now() - interval '450 days') GROUP BY date_15min) AS average_15min_calc WHERE average_15min_calc.date_15min = pr_15min_cau3.date_15min;
...
INSERT INTO pr_60min_cau3(date_60min) SELECT (to_timestamp(ceiling(( extract ('epoch' from (date)) ) / (60*60) ) * (60*60))) AS date_60min from pr_cau3 WHERE date >= (now() - interval '450 days') GROUP BY date_60min;
...
UPDATE pr_60min_cau3 SET g_60min_average = average_60min_calc.average_60min 
	FROM (SELECT 
		...
		(to_timestamp(ceiling(( extract ('epoch' from (date)) ) / (60*60) ) * (60*60))) AS date_60min
			from pr_cau3 WHERE date >= (now() - interval '450 days') GROUP BY date_60min) AS average_60min_calc WHERE average_60min_calc.date_60min = pr_60min_cau3.date_60min;
...
INSERT INTO pr_24hs_cau3 SELECT date_trunc('day', date) FROM pr_cau3 WHERE date >= (now() - interval '450 days') GROUP BY date_trunc('day', date);

UPDATE pr_24hs_cau3 SET g_24hs_sum = g_24hs_sum_calc.sum FROM (SELECT date_trunc('day', date), SUM(g_average_m2) FROM pr_cau3 WHERE date >= (now() - interval '450 days') GROUP BY date_trunc('day', date)) AS g_24hs_sum_calc WHERE g_24hs_sum_calc.date_trunc = pr_24hs_cau3.date_24hs ;
...
------------------MERGE-----------------------
UPDATE pr_cau3 SET dia = date::date WHERE date >= (now() - interval '450 days');
...
UPDATE pr_cau3 SET "g_3h_average_600_consecutivas " = '0' WHERE date >= (now() - interval '450 days') AND g_3h_average_consecutivas < 600;
UPDATE pr_cau3 SET "g_3h_average_600_consecutivas " = '1' WHERE date >= (now() - interval '450 days') AND g_3h_average_consecutivas >= 600;
...
INSERT INTO pr_check_600_3h_consecutivas(dia) SELECT DISTINCT date::date FROM pr_cau3 WHERE date >= (now() - interval '450 days');

UPDATE pr_check_600_3h_consecutivas SET check_average = check_average_calc.check_average 
	FROM (SELECT (AVG("g_3h_average_600_consecutivas ")) AS check_average, (date_trunc('day', date))::date AS date_1dia FROM pr_cau3 WHERE date >= (now() - interval '450 days') GROUP BY date_trunc('day', date)) AS check_average_calc WHERE check_average_calc.date_1dia = pr_check_600_3h_consecutivas.dia ;
