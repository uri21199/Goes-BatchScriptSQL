...
INSERT INTO consolidador_arb_temp(date, exportada_cammesa) SELECT DATE_TRUNC('hour',date - interval '15 minutes') as date2, SUM(exportada_ARBEM21P) FROM smec.arb_columnas WHERE date >= (now() - interval '20 days') GROUP BY date2 order by date2;
...
SELECT * from smec.arb_ids CROSS JOIN generate_series(DATE_TRUNC('hour', now() - interval '21 day') ,DATE_TRUNC('hour', now()), '1 hour')  ON CONFLICT DO NOTHING;

----------------------------------------------------------------------------

------------------------------------------------------------------------------------------------------------------------------------------
UPDATE smec.consolidador_arb SET exportada_cammesa = exportada_cammesa/1000 WHERE date >= (now() - interval '20 days');
UPDATE smec.consolidador_arb SET po_exportada = s.a FROM (SELECT date_hour - interval '1 hour' as date, grupo as id, egenerada as a FROM smec.po_general_renovables WHERE grupo LIKE 'ABEFDI01') as s WHERE consolidador_arb.date = s.date AND consolidador_arb.arb = s.id AND consolidador_arb.date >= (now() - interval '20 days');


--------------------------------------------------------------------------------------------------------------------------------------------

UPDATE smec.consolidador_arb SET cammesa_ppa = s.a FROM (SELECT exportada_cammesa as a, date, arb FROM smec.consolidador_arb WHERE exportada_cammesa <= 1.5 ) as s WHERE s.date = consolidador_arb.date AND s.arb = consolidador_arb.arb AND consolidador_arb.date >= (now() - interval '20 days');
UPDATE smec.consolidador_arb SET cammesa_spot = exportada_cammesa - 1.5 WHERE date >= (now() - interval '20 days');
UPDATE smec.consolidador_arb SET cammesa_spot = 0 WHERE cammesa_spot <= 0 AND date >= (now() - interval '20 days');
UPDATE smec.consolidador_arb SET cammesa_ppa = 1.5 WHERE exportada_cammesa >= 1.5 AND date >= (now() - interval '20 days');

UPDATE smec.consolidador_arb SET po_ppa = s.a FROM (SELECT po_exportada as a, date, arb FROM smec.consolidador_arb WHERE po_exportada <= 1.5 ) as s WHERE s.date = consolidador_arb.date AND s.arb = consolidador_arb.arb AND consolidador_arb.date >= (now() - interval '20 days');
UPDATE smec.consolidador_arb SET po_spot = po_exportada - 1.5 WHERE date >= (now() - interval '20 days');
UPDATE smec.consolidador_arb SET po_spot = 0 WHERE po_spot <= 0 AND date >= (now() - interval '20 days');
UPDATE smec.consolidador_arb SET po_ppa = 1.5 WHERE po_exportada >= 1.5 AND date >= (now() - interval '20 days');


-----------PRN FALTANTE FIX--------

update smec.consolidador_arb set exportada_cammesa = po_exportada where date >= '2024-05-09 00:00:00-03' and date <= '2024-05-09 23:00:00-03' and exportada_cammesa is null;

------------------------------------

alter table arrebeef.decomiso add column dolar_habil numeric;
alter table arrebeef.decomiso add column ahorrado_dolar numeric;

update arrebeef.decomiso set dolar_habil = venta from smec.dolar_habil where dolar_habil.periodo = decomiso."Fecha";
update arrebeef.decomiso set dolar_habil = s.a from (select venta as a, periodo from smec.dolar_habil order by periodo desc limit 1) as s where dolar_habil is null;
update arrebeef.decomiso set ahorrado_dolar = " $ ahorrada " / dolar_habil;

update smec.consolidador_arb set dolar_habil = dolar_habil.venta from smec.dolar_habil where date_trunc('month', consolidador_arb.date::date) = dolar_habil.periodo;
update smec.consolidador_arb set dolar_habil = s.a from (select venta as a, periodo from smec.dolar_habil order by periodo desc limit 1) as s where dolar_habil is null;
update smec.consolidador_arb set exportada_cammesa_dolar = 0;
update smec.consolidador_arb set exportada_cammesa_dolar = exportada_cammesa / dolar_habil;
