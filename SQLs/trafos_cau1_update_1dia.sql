...
BEGIN
    FOR rec IN SELECT * FROM trafos_sources LOOP
        ...
        EXECUTE format(
            'COPY stage_trafos_cau_1 FROM PROGRAM ''curl -H "Api-Key:38deaa51-8716-4cc9-aa42-ab14fb72a752.01" "%s?&start_date=%sT%sZ"''',
            rec.url, startdate, starttime
        );

        -- Insertar los datos en la tabla temporal y hacer las actualizaciones
        INSERT INTO stage_trafos_cau_12 
        SELECT f.* 
        FROM stage_trafos_cau_1, jsonb_populate_recordset(null::stage_trafos_cau_12, x) f;

        UPDATE stage_trafos_cau_12 
        SET cau = 'cau1fv', 
            id = rec.id,
            date = DATE_TRUNC('minute', date);

        -- Insertar los datos finales en la tabla objetivo
        INSERT INTO alert_cau.caupotenciatrafos_cau1fv
        SELECT * 
        FROM stage_trafos_cau_12 
