...
-- Ingreso de datos y actualizacion de otros campos
...
INSERT INTO cafayate.cafdatabasehf(date) SELECT date from cafayate.caf_wind_speed WHERE date > now() - interval '8 hours' ON CONFLICT ON CONSTRAINT uc_cafdatabasehf do nothing;
...
UPDATE Cafayate.cafdatabasehf SET mod_temp4 = s.a FROM (SELECT date, val as a FROM cafayate.mod_temp WHERE id = 'mod temp 4' AND date > now() - interval '8 hours') as s WHERE s.date = cafdatabasehf.date AND cafdatabasehf.date > now() - interval '8 hours';
UPDATE Cafayate.cafdatabasehf SET mod_temp9 = s.a FROM (SELECT date, val as a FROM cafayate.mod_temp WHERE id = 'mod temp 9' AND date > now() - interval '8 hours') as s WHERE s.date = cafdatabasehf.date AND cafdatabasehf.date > now() - interval '8 hours';
